...
def test_junit_parser_can_load_junit_output():
    test_xml = load_test_results('tests_unit/TEST-Junit.xml')
...
def test_junit_parser_can_validate_junit_output():
    (control_result, message) = is_compliant_test_results('tests_unit/TEST-Junit.xml')
    assert control_result is False
    assert message == "Tests contain failures"


def test_junit_parser_can_load_pytest_output():
    test_xml = load_test_results('tests_unit/TEST-Pytest-pass.xml')
    # one test suite in file
...
def test_junit_parser_can_load_pytest_failed_output():
    test_xml = load_test_results('tests_unit/TEST-Pytest-fail.xml')
    # one test suite in file
    assert test_xml._tag == "testsuites"
    assert len(test_xml) == 1
    assert test_xml.failures == 1
...
def test_junit_parser_can_validate_pytest_failed_output():
    (control_result, message) = is_compliant_test_results('tests_unit/TEST-Pytest-fail.xml')
    assert control_result is False
    assert message == "Tests contain failures"


def test_junit_parser_can_load_owasp_output():
    test_xml = load_test_results('tests_unit/TEST-OWASP-pass.xml')
    # one test suite in file
    assert test_xml._tag == "testsuites"
...
def test_junit_parser_can_load_owasp_failed_output():
    test_xml = load_test_results('tests_unit/TEST-OWASP-fail.xml')
    # one test suite in file
    assert test_xml._tag == "testsuites"
...
def test_junit_parser_can_validate_owasp_failed_output():
    (control_result, message) = is_compliant_test_results('tests_unit/TEST-OWASP-fail.xml')
    assert control_result is False
    assert message == "Tests contain failures"


def test_junit_parser_can_load_surefire_output():
    test_xml = load_test_results('tests_unit/surefire_examples/TEST-com.compliancedb.example.Example1Test.xml')
    # one test suite in file
    assert test_xml._tag == "testsuite"
...
def test_list_test_result_files():
    files = ls_test_results('tests_unit/surefire_examples/')
    assert files == ['tests_unit/surefire_examples/TEST-com.compliancedb.example.Example1Test.xml',
                     'tests_unit/surefire_examples/TEST-com.compliancedb.example.Example2Test.xml']


def test_list_test_result_files_filters_failsafe_summary_xml():
    files = ls_test_results('tests_unit/failsafe_examples/')
    assert files == ['tests_unit/failsafe_examples/TEST-com.compliancedb.example.Example1Test.xml']
...
def test_is_compliant_tests_directory_passes_if_every_file_has_no_failures(mocked_ls, mocked_results):
    mocked_ls.return_value = \
        ['tests_unit/surefire_examples/TEST-com.compliancedb.example.Example1Test.xml',
         'tests_unit/surefire_examples/TEST-com.compliancedb.example.Example2Test.xml']

    mocked_results.side_effect = [(True, "All good"), (True, "All good")]
    (result, message) = is_compliant_tests_directory("tests_unit/surefire_examples")
...
def test_passing_failsafe_testxml_results_in_compliant_evidence():
    test_xml = load_test_results('tests_unit/failsafe_examples/TEST-com.compliancedb.example.Example1Test.xml')
    (control_result, message) = is_compliant_suite(test_xml)
    assert control_result is True
    assert message == "All tests passed"
